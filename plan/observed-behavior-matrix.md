...
| P2a | Setext heading | Slug should match heading text. | Works in paired `test-gfm.md` / `test-native.md` fixtures; Obsidian's `HeadingCache` strips the setext underline so `heading.heading` is plain text; `slugify`, `buildSlugTable`, and the new `stripMarkdown` setext-underline regex all produce the expected slug. | GREEN | Navigation reaches the correct heading; middle-click flicker/latency is tracked in the QA-log follow-up items. |
...
| P2a | Heading with parentheses, e.g. `## Function foo(bar)` | Match GFM slug. | Works in paired `test-gfm.md` / `test-native.md` fixtures; `github-slugger`, `slugify`, and `buildSlugTable` produce `function-foobar`. | GREEN | Navigation reaches the correct heading; middle-click flicker/latency is tracked in the QA-log follow-up items. |
...
| P2a | Heading with image, e.g. `## ![alt](img.png) Title` | Slug should ignore or handle image consistently with GitHub. | Works in paired `test-gfm.md` / `test-native.md` fixtures; `stripMarkdown` removes the image entirely, and `slugify` plus `buildSlugTable` produce `title`. macOS QA: Reading, Live Preview rendered, Live Preview source text, and Source mode match native across plain click and Cmd-click. | GREEN | Verified on macOS desktop. Middle-click was not rerun for this issue; the broader middle-click matrix remains based primarily on Windows QA. GitHub parity accepted via `github-slugger` because the Markdown rendering API omits generated heading IDs. |
