...
| P2a | Heading with square brackets, e.g. `## API [v2]` | Match GFM slug for bracketed text. | Works in paired `test-gfm.md` / `test-native.md` fixtures; `github-slugger`, `slugify`, and `buildSlugTable` produce `api-v2`. | GREEN | Middle-click in Live Preview rendered/unrendered and Source mode still shows the known flicker/latency follow-up, but navigation reaches the correct heading. |
